...
#### L'absence d'argument correspond à la France entière.
...
draw_map <- function(input_year, region, mapped_var, mapped_var_name, base_width) {
  ...
  ## La carte est recalculée pour conserver l'échelle du texte.
  ## Mais la fonction bugue inexplicablement pour la France.
  ## Il faut redémarrer R, ce qui est très pénible.
  ## D'où l'exclusion suivante.
  if (region != 0) {
    display_plot <- ggplot() +
      geom_sf(data = carto, aes(fill = .data[[mapped_var]])) +
      scale_fill_viridis_c(name = mapped_var_name, labels = scales::percent_format(scale = 100)) +
...
Tableau présentant le nombre de NA par variable et par an, coloré façon heatmap.
...
```{r}
variables_of_interest <- c(mca_regressors, relative_outcomes)

mca_data_subset <- mca_data[, ..variables_of_interest]
mca_data_subset <- na.omit(mca_data_subset)

result <- PCA(mca_data_subset, graph = TRUE)  # Setting graph = FALSE to prevent automatic graph plotting
