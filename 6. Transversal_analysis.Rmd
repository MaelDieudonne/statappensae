...
freq_row_index <- which(t_means$cluster == "Freq")
t_means$cluster[freq_row_index] <- "Effectifs"
t_means <- rbind(t_means[freq_row_index, ], t_means[-freq_row_index, ])
...
means_tbl <- t_means %>%
  ...
  data_color(method = "numeric", columns = !matches("cluster|mean"), direction = "row", palette = "RColorBrewer::RdBu") %>%
  tab_spanner(label = "Clusters", columns = c("1", "2", "3", "4", "5", "6"), id = "clust") %>%
  cols_label(cluster = "", mean = "Moyenne") %>%
  tab_row_group(label = "", rows = 1, id = "n") %>%
  tab_row_group(label = "Outcomes", rows = 2:16, id = "out") %>%
  tab_row_group(label = "Régresseurs", rows = 17:45, id = "reg") %>%
  row_group_order(groups = c("n", "out", "reg")) %>%
  sub_missing(
    columns = everything(),
    rows = everything(),
    missing_text = "-") %>%
  tab_style(
    style = list(
      cell_fill(color = "lightgrey"),
      cell_text(style = "italic", align = "center", v_align = "top")),
    locations = cells_row_groups(groups = c("n", "out", "reg"))) %>%
  tab_style(
    style = list(
      cell_text(style = "italic", align = "center")),
    locations = list(
...
# Nettoyage
rm(regressors, mca_data_subset, dummies, kmeans_result, variables_of_interest, means, cluster_counts, global_means, t_means, means_tbl, mca_data_subset_scaled, scaling_regressors, freq_row_index)
```
