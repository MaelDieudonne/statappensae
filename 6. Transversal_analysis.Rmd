...
means_tbl <- t_means %>%
  gt() %>%
  fmt_number(rows = 1, decimals = 0, scale_by = 1000) %>%
  fmt_number(rows = 2, decimals = 0) %>%
  fmt_number(rows = 3:46, decimals = 2, drop_trailing_zeros = TRUE) %>%
  fmt_number(rows = (c(40,42)), decimals = 0, drop_trailing_zeros = TRUE) %>%
...
# Nettoyage
rm(cluster_counts, cluster_data, columns, dummies, first_rows, global_means, kmeans_result, last_rows, mca_data_subset, mca_data_subset_scaled, means, means_tbl, regressors, scaling_regressors, sum, t_means, variables_of_interest)
```
