...
global_means$cluster <- "weighted_mean"
means <- rbind(means, global_means, fill = TRUE)
...
means_tbl <- t_means %>%
  ...
  data_color(
    method = "numeric",
    columns = !matches("cluster|mean|weighted_means"),
    rows = 3:46,
    direction = "row",
    palette = "RColorBrewer::RdBu",
    alpha = 0.6) %>%
  tab_spanner(label = "Clusters", columns = c("1", "2", "3", "4", "5", "6"), id = "clust") %>%
  tab_spanner(label = "Moyennes", columns = c("mean", "weighted_mean"), id = "moy") %>%
  cols_label(cluster = "", mean = "Comm.", weighted_mean = "Pop.") %>%
  ...
  tab_style(
    style = list(
      cell_text(style = "italic", align = "center")),
    locations = list(
      cells_column_labels(columns = c("mean", "weighted_mean")), 
      cells_body(columns = c("mean", "weighted_mean")))) %>%
 tab_style(
    style = list(
      cell_text(align = "center")),
    locations = list(
      cells_column_labels(columns = c("1", "2", "3", "4", "5", "6", "mean", "weighted_mean")),
