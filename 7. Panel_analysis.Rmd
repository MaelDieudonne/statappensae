...
absolute_outcomes <- c("GIns_ratio_T1", "DIns_ratio_T1", "EIns_ratio_T1", "CIns_ratio_T1")
absolute_outcomes_2T <- c("GIns_ratio_T1", "DIns_ratio_T1", "GIns_ratio_T2", "DIns_ratio_T2", "EIns_ratio_T1", "CIns_ratio_T1", "EIns_ratio_T2", "CIns_ratio_T2")
...
variable_names = c(relative_outcomes_2T, full_panel_regressors)
...
na_prop_table <- na_prop_table %>%
  replace(., . == 100, NaN) %>%
  t() %>%
  `colnames<-`(.[1, ]) %>%
  .[-1, ]

max_value <- max(na_prop_table[is.finite(na_prop_table)])
bins <- seq(0, max_value, by = max_value/9)

gt_na_prop <- na_prop_table %>%
  as.data.frame() %>%
  gt(., rownames_to_stub = TRUE) %>%
  fmt_number(decimals = 2, drop_trailing_zeros = TRUE) %>%
  tab_header(title = "Proportion de valeurs manquantes", subtitle = NULL, preheader = NULL) %>%
  data_color(method = "bin", bins = bins, direction = "column", palette ="Oranges", na_color = "white", alpha = 0.6) %>%
  tab_row_group(label = "Outcomes", rows = 1:6, id = "out") %>%
  tab_row_group(label = "Régresseurs", rows = 7:27, id = "reg") %>%
...
gt_na_prop
gtsave(gt_na_prop, filename = "nan_table.tex", path = "output_panel")

rm(count_na, variable_names, na_prop_table, max_value, bins, gt_na_prop, name, na_prop)
```
...
plot <- ggplot(graph_data, aes(x = year)) +
  ...
  labs(
    title = "Evolution des paramètres d'intérêt",
    subtitle = "1965-2022",
    x = "Années",
    y = "Pourcentage",
    color = "Outcomes") +
...
for (year in 1960:2022) {
  ...
  average_ppropri <- 100 * mean(propri[[col_ppropri]], na.rm = TRUE)
  prop_ratio <- 100 * sum_npropri / sum_nlogement
...
plot <- ggplot(propri_df, aes(x = year)) +
  ...
  labs(
    title = "Taux de propriétaires de leur résidence principale",
    subtitle = "1960-2022",
    x = "Années",
    y = "",
    color = "") +
...
plot <- ggplot(quantiles_df, aes(x = year, y = ppropri, color = as.factor(quintile), group = quintile)) +
  geom_line() +
  labs(
    title = "Déciles du taux communal moyen de propriétaires",
    x = "Années", 
    y = "", 
    color = "Déciles") +
  theme_grey() +
...
Pour terminer, un ridgeline représentant plus finement l'évolution de la distribution du taux de propriétaires.

```{r}
plot <- ggplot(panel_data, aes(x = ppropri, y = as.factor(year), fill = after_stat(x))) +
  geom_density_ridges_gradient(scale = 4, rel_min_height = 0.01) +
  scale_fill_viridis(option = "C") +
  labs(
    title = "Distribution du taux de propriétaires",
    subtitle = "1965 - 2022",
    x = "",
    y = "") +
  theme_grey() +
  theme(
    ...
    plot.subtitle = element_text(hjust = 0.5))
print(plot)
ggsave("output_panel/ridge_ppropri_1965_2022.png", plot, width = 8, height = 6, dpi = 300)
...
On utilise deux tests :

-   Le test de Wooldrige à partir des différences premières, qui concerne la corrélation temporelles des résidus.

-   Le test de Hausman, qui permet de comparer les estimateurs fd et within, donc d'évaluer l'hypothèse d'exogénéité stricte.

```{r}
# Hausman test
# Comparaison des estimateurs fd et within => H0 correspond à l'exogénéité stricte
panel_as_pdata <- pdata.frame(panel_data, index = c("codecommune", "year"))

print("Test for part_T1")
regression_formula <- as.formula(paste("part_T1 ~", paste(panel_regressors, collapse = "+")))
pwfdtest(regression_formula, data = panel_as_pdata)
fd <- plm(regression_formula, data = panel_as_pdata, model = "fd", effect = "individual", cluster = "dep", vcov = "HC1")
within <- plm(regression_formula, data = panel_as_pdata, model = "within", effect = "individual", cluster = "dep", vcov = "HC1")
phtest(fd, within)

print("Test for GD_ratio_T1")
...
### Avec dummies temporelles et interactions
...
```{r}
get_coeffs <- function(model, outcome_name) {
  coefficients <- as.data.frame(model$coefficients)
  coefficients <- coefficients[grep("ppropri$", rownames(coefficients)), , drop = FALSE]
  colnames(coefficients) <- "coef_ppropri"
  years <- sub("^year(\\d+):.*", "\\1", rownames(coefficients))
  coefficients$year <- years
  coefficients$outcome <- outcome_name
  
  variance <- as.data.frame(model$vcov)
  variance <- variance[grep("ppropri$", rownames(variance)), grep("ppropri$", colnames(variance)), drop = FALSE]
  variance <- diag(as.matrix(variance))
  variance <- as.data.frame(variance)
  colnames(variance) <- "var"
  
  coefficients <- cbind(coefficients, variance)
  coefficients$CI_low <- coefficients$coef_ppropri - 1.96 * sqrt(coefficients$var)
  coefficients$CI_up <- coefficients$coef_ppropri + 1.96 * sqrt(coefficients$var)
  
  return(coefficients)
}

part_T1 <- get_coeffs(relative_results$part_T1_model, "part_T1")
GD_ratio_T1 <- get_coeffs(relative_results$GD_ratio_T1_model, "GD_ratio_T1")
EC_ratio_T1 <- get_coeffs(relative_results$EC_ratio_T1_model, "EC_ratio_T1")
coefficients <- rbind(part_T1, GD_ratio_T1, EC_ratio_T1)
coefficients$year <- as.numeric(coefficients$year)

plot <- ggplot(coefficients) +
  geom_line(aes(x = year, y = coef_ppropri, group = outcome, color = outcome)) +
  geom_line(aes(x = year, y = CI_low, group = outcome, color = outcome, ), linetype = "dashed", alpha = 0.5) +
  geom_line(aes(x = year, y = CI_up, group = outcome, color = outcome, ), linetype = "dashed", alpha = 0.5) + 
  geom_ribbon(aes(x = year, ymin = CI_low, ymax = CI_up, group = outcome, fill = outcome), alpha = 0.3) +
  geom_hline(yintercept = 0, linetype = "twodash", color = "gray40") + 
  scale_color_manual(
    values = c("part_T1" = "blue", "GD_ratio_T1" = "darkorange", "EC_ratio_T1" = "limegreen"),
    labels = c("part_T1" = "Participation", "GD_ratio_T1" = "GD Ratio", "EC_ratio_T1" = "EC Ratio")) +
  scale_fill_manual(
    values = c("part_T1" = "skyblue", "GD_ratio_T1" = "orange", "EC_ratio_T1" = "lightgreen"),
    labels = c("part_T1" = "Participation", "GD_ratio_T1" = "GD Ratio", "EC_ratio_T1" = "EC Ratio")) +
  scale_x_continuous(breaks = unique(coefficients$year)) +
  labs(
    title = "Coefficients de ppropri dans les panels",
    subtitle = "(avec interactions temporelles)",
    x = "Années",
    y = "",
    color = "Coefficients",
    fill = "95 % IC") +
...
rm(part_T1, GD_ratio_T1, EC_ratio_T1, coefficients, plot)
```
...
Comme précédemment, on obtient des R2 très faibles, bien que marginalement meilleurs.
...
part <- get_coeffs(relative_results$part_model, "part")
GD_ratio <- get_coeffs(relative_results$GD_ratio_model, "GD_ratio")
EC_ratio <- get_coeffs(relative_results$EC_ratio_model, "EC_ratio")
coefficients <- rbind(part, GD_ratio, EC_ratio)
coefficients$year <- as.numeric(coefficients$year)
...
plot <- ggplot(coefficients) +
  ...
  geom_hline(yintercept = 0, linetype = "twodash", color = "gray40") + 
  scale_color_manual(
    values = c("part" = "blue", "GD_ratio" = "darkorange", "EC_ratio" = "limegreen"),
    labels = c("part" = "Participation", "GD_ratio" = "GD Ratio", "EC_ratio" = "EC Ratio")) +
  scale_fill_manual(
    values = c("part" = "skyblue", "GD_ratio" = "orange", "EC_ratio" = "lightgreen"),
    labels = c("part" = "Participation", "GD_ratio" = "GD Ratio", "EC_ratio" = "EC Ratio")) +
  scale_x_continuous(breaks = unique(coefficients$year)) +
  labs(
    title = "Coefficients de ppropri dans les panels",
    subtitle = "(avec interactions temporelles et tours confondus)",
    x = "Années",
    y = "",
    color = "Coefficients",
    fill = "95 % IC") +
  ...
  theme(
    ...
    panel.grid.minor = element_blank())
print(plot)
ggsave("output_panel/ppropri_reg_coeffs.png", plot, width = 8, height = 6, dpi = 300)

rm(part, GD_ratio, EC_ratio, coefficients, plot)
```
...
```{r}
results <- data.frame(year = integer(), "part_T1" = numeric(), "part_T2" = numeric(), "GD_ratio_T1" = numeric(), "GD_ratio_T2" = numeric(), "EC_ratio_T1" = numeric(), "EC_ratio_T2" = numeric(), "R2_adj" = numeric(), "CI_low" = numeric(), "CI_up" = numeric())
...
for (outcome in relative_outcomes) {
  for (yr in unique(panel_data$year)) {
    ...
    tryCatch({
      ...
      results <- rbind(results, data.frame(outcome = outcome, year = yr, coef_ppropri = coef(model)["ppropri"], CI_low = (confint(model)["ppropri", 1]), CI_up = (confint(model)["ppropri", 2]), r2_adj = summary(model)$adj.r.squared))
      }, error = function(e) {
...
ggplot(results, aes(x = year, y = r2_adj, color = outcome, group = outcome)) +
  ...
  scale_color_manual(
    ...
    labels = c("part_T1" = "Participation", "GD_ratio_T1" = "GD Ratio", "EC_ratio_T1" = "EC Ratio")) +
  labs(
    title = "R2 ajusté des régressions annuelles",
    x = "Années",
    y = "R2 ajusté",
    color = "Outcome") +
  theme_grey() +
...
plot <- ggplot(results) +
  ...
  scale_fill_manual(
    ...
    labels = c("part_T1" = "Participation", "GD_ratio_T1" = "GD Ratio", "EC_ratio_T1" = "EC Ratio")) +
  labs(
    title = "Coefficients de ppropri dans les régressions annuelles",
    x = "Années",
    y = "",
   color = "Coefficient",
   fill = "95 % IC") +
  ...
  theme(
    ...
    panel.grid.minor = element_blank())
print(plot)
ggsave("output_panel/ppropri_panel_coeffs_abs.png", plot, width = 8, height = 6, dpi = 300)

rm(results, outcome, yr, subset_data, regression_formula, model, plot)
```

## Pseudo-quantiles

En considérant les quantiles de ppropri plutôt que de l'outcome.
...
pseudo_quant_reg <- function(outcome) {
  ...
  plot_r2 <- ggplot(R2_df, aes(x = year, y = R2, color = quantile, group = quantile)) +
    geom_line() +
    geom_hline(yintercept = 0, linetype = "twodash", color = "gray40") + 
    scale_x_continuous(breaks = unique(R2_df$year)) + 
    labs(
      title = paste("R2 des régressions pseudo-quantiles de", outcome),
      subtitle ="(quartiles mobiles)",
      x = "Années",
      y = "R2",
      color = "Quartiles") +
    theme_grey() +
  ...
  output_file <- paste0("output_panel/pseudo_quant_abs_", outcome, ".png")
  plot_coef <- ggplot(coefs_df, aes(x = year, y = coefficient, color = quantile, group = quantile)) +
    geom_line() +
    geom_hline(yintercept = 0, linetype = "twodash", color = "gray40") + 
    scale_x_continuous(breaks = unique(coefs_df$year)) +
    labs(
      title = paste("Coefficients de ppropri dans les régressions pseudo-quantiles de", outcome),
      subtitle ="(quartiles mobiles)",
      x = "Années",
      y = "Coefficients",
      color = "Quartiles") +
    ...
    theme(
      ...
      panel.grid.minor = element_blank())
  print(plot_coef)
  ggsave(output_file, plot_coef, width = 8, height = 6, dpi = 300)
}
...
pseudo_quant_reg <- function(outcome) {
  ...
  plot_r2 <- ggplot(R2_df, aes(x = year, y = R2, color = quantile, group = quantile)) +
    ...
    scale_x_continuous(breaks = unique(R2_df$year)) +  
    labs(
      title = paste("R2 des régressions pseudo-quantiles de", outcome), subtitle ="(quantiles absolus)",
      x = "Années",
      y = "R2",
      color = "Quantiles") +
    theme_grey() +
  ...
  output_file <- paste0("output_panel/pseudo_quant_rel_", outcome, ".png")
  plot_coef <- ggplot(coefs_df, aes(x = year, y = coefficient, color = quantile, group = quantile)) +
    geom_line() +
    geom_hline(yintercept = 0, linetype = "twodash", color = "gray40") + 
    scale_x_continuous(breaks = unique(coefs_df$year)) + 
    labs(
      title = paste("Coefficients de ppropri dans les régressions pseudo-quantiles de", outcome), subtitle ="(quantiles absolus)",
      x = "Années",
      y = "Coefficients",
      color = "Quantiles") +
    theme_grey() +
...
```{r}
ppropri_df <- data.frame(year = integer(), sum_comm = numeric(), sum_pop = numeric())

for (yr in unique(panel_data$year)) {
  panel_subset <- panel_data[panel_data$year == yr, ]
  panel_subset <- panel_subset[panel_subset$ppropri >= 95, ]
  sum_comm <- nrow(panel_subset)
  sum_pop <- sum(panel_subset$popcomm, na.rm = TRUE)
  ppropri_df <- rbind(ppropri_df, data.frame(year = yr, sum_comm = sum_comm, sum_pop = sum_pop))
}

plot_counts <- ggplot(ppropri_df, aes(x = year, y = sum_comm)) +
  geom_line(color = "skyblue") +
  scale_x_continuous(breaks = unique(ppropri_df$year)) +  
  labs(
    title = "Nombre de communes où le taux de propriétaires est >95%",
    x = "Années",
    y = "Effectifs") +
  theme_grey() +
  theme(
      ...
      panel.grid.minor = element_blank())
print(plot_counts)
ggsave("output_panel/ppropri_max.png", plot_counts, width = 8, height = 6, dpi = 300)

rm(ppropri_df, plot_counts, yr, sum_comm, sum_pop)
```
...
quant_reg <- function(outcome) {
  ...
  output_file <- paste0("output_panel/quant_abs_", outcome, ".png")
  plot_coef <- ggplot(coefs_df, aes(x = year, y = coefficient, color = factor(quantile))) +
    geom_line() +
    geom_hline(yintercept = 0, linetype = "twodash", color = "gray40") + 
    scale_x_continuous(breaks = unique(coefs_df$year)) +   
    labs(
      title = paste("Coefficients de ppropri dans les régressions quantiles de", outcome),
      x = "Années",
      y = "Coefficients") +
...
quant_reg <- function(outcome) {
  ...
  for (yr in unique(panel_data$year)) {
    panel_subset <- panel_data[panel_data$year == yr, ]
    for (range_name in names(outcome_ranges)) {
      range_values <- outcome_ranges[[range_name]]
      quantile_subset <- panel_subset[panel_subset[[outcome]] >= range_values[1] & panel_subset[[outcome]] < range_values[2], ]
      tryCatch({
        model <- lm(regression_formula, data = quantile_subset)
        coefs_df <- rbind(coefs_df, data.frame(year = yr, quantile = range_name, coefficient = model$coef["ppropri"]))
        R2_df <- rbind(R2_df, data.frame(year = yr, quantile = range_name, R2 = summary(model)$adj.r.squared))
      }, error = function(e) {
        coefs_df <- rbind(coefs_df, data.frame(year = yr, quantile = range_name, coefficient = NA))
        R2_df <- rbind(R2_df, data.frame(year = yr, quantile = range_name, R2 = NA))
      })
    }
  }
  ...
  output_file <- paste0("output_panel/quant_rel_", outcome, ".png")
  plot_coef <- ggplot(coefs_df, aes(x = year, y = coefficient, color = quantile, group = quantile)) +
    geom_line() +
    geom_hline(yintercept = 0, linetype = "twodash", color = "gray40") + 
    scale_x_continuous(breaks = unique(coefs_df$year)) + 
