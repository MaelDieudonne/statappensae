...
part_T1_var <- as.data.frame(relative_results$part_T1_model$vcov)
part_T1_var <- part_T1_var[grep("ppropri$", rownames(part_T1_var)), grep("ppropri$", colnames(part_T1_var)), drop = FALSE]
part_T1_var <- diag(as.matrix(part_T1_var))
part_T1_var <- as.data.frame(part_T1_var)
colnames(part_T1_var) <- "var"

part_T1 <- cbind(part_T1, part_T1_var)
part_T1$CI_low <- part_T1$coef_ppropri - 1.96*sqrt(part_T1$var)
part_T1$CI_up <- part_T1$coef_ppropri + 1.96*sqrt(part_T1$var)

ggplot(part_T1) +
  geom_line(aes(x = year, y = coef_ppropri, color = "coef", group = outcome)) +
  geom_line(aes(x = year, y = CI_low, group = outcome), color = "limegreen", linetype = "dashed") +
  geom_line(aes(x = year, y = CI_up, group = outcome), color = "limegreen", linetype = "dashed") +
  geom_ribbon(aes(x=year, ymin = CI_low, ymax = CI_up, , group = outcome, fill = "IC"), alpha = 0.5) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") + 
  scale_color_manual(values = c("coef" = "cornflowerblue")) +
  scale_fill_manual(values = c("IC" = "gray")) +
  labs(title = "Coefficients de ppropri dans les panels - part T1", subtitle = "(avec interactions temporelles)",
       x = "Année",
       y = "Coefficient") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5), legend.position = "none")

rm(part_T1, part_T1,var, years)
```
...
GD_ratio_T1_var <- as.data.frame(relative_results$GD_ratio_T1_model$vcov)
GD_ratio_T1_var <- GD_ratio_T1_var[grep("ppropri$", rownames(GD_ratio_T1_var)), grep("ppropri$", colnames(GD_ratio_T1_var)), drop = FALSE]
GD_ratio_T1_var <- diag(as.matrix(GD_ratio_T1_var))
GD_ratio_T1_var <- as.data.frame(GD_ratio_T1_var)
colnames(GD_ratio_T1_var) <- "var"

GD_ratio_T1 <- cbind(GD_ratio_T1, GD_ratio_T1_var)
GD_ratio_T1$CI_low <- GD_ratio_T1$coef_ppropri - 1.96*sqrt(GD_ratio_T1$var)
GD_ratio_T1$CI_up <- GD_ratio_T1$coef_ppropri + 1.96*sqrt(GD_ratio_T1$var)
...
ggplot(GD_ratio_T1) +
  ...
  scale_color_manual(values = c("coef" = "cornflowerblue")) +
  scale_fill_manual(values = c("IC" = "gray")) +
  labs(title = "Coefficients de ppropri dans les panels - GD ratio T1", subtitle = "(avec interactions temporelles)",
       x = "Année",
       y = "Coefficient") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5), legend.position = "none")

rm(GD_ratio_T1, GD_ratio_T1,var, years)
```
...
EC_ratio_T1_var <- as.data.frame(relative_results$EC_ratio_T1_model$vcov)
EC_ratio_T1_var <- EC_ratio_T1_var[grep("ppropri$", rownames(EC_ratio_T1_var)), grep("ppropri$", colnames(EC_ratio_T1_var)), drop = FALSE]
EC_ratio_T1_var <- diag(as.matrix(EC_ratio_T1_var))
EC_ratio_T1_var <- as.data.frame(EC_ratio_T1_var)
colnames(EC_ratio_T1_var) <- "var"

EC_ratio_T1 <- cbind(EC_ratio_T1, EC_ratio_T1_var)
EC_ratio_T1$CI_low <- EC_ratio_T1$coef_ppropri - 1.96*sqrt(EC_ratio_T1$var)
EC_ratio_T1$CI_up <- EC_ratio_T1$coef_ppropri + 1.96*sqrt(EC_ratio_T1$var)
...
ggplot(EC_ratio_T1) +
  ...
  scale_color_manual(values = c("coef" = "cornflowerblue")) +
  scale_fill_manual(values = c("IC" = "gray")) +
  labs(title = "Coefficients de ppropri dans les panels - EC ratio T1", subtitle = "(avec interactions temporelles)",
       x = "Année",
       y = "Coefficient") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5), legend.position = "none")

rm(EC_ratio_T1, EC_ratio_T1,var, years)
```
