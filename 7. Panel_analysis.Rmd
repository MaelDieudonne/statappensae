...
plot <- ggplot(propri_df, aes(x = year)) +
  ...
  labs(x = "Années", y = "", color = "") +
  scale_color_manual(
    values = c("prop_ratio" = "skyblue", "ppropri" = "orange"),
...
plot <- ggplot(coefficients) +
  ...
  theme(panel.grid.minor = element_blank())
print(plot)
ggsave("output_panel/ppropri_panels_coeffs_rounds.png", plot, width = 8, height = 6, dpi = 300)
...
plot <- plot + labs(title = "Coefficients de ppropri dans les régressions annuelles")
print(plot)
...
pseudo_quant_reg <- function(outcome) {
  ...
  output_file <- paste0("output_panel/pseudo_quant_mob_", outcome, ".png")
...
pseudo_quant_reg <- function(outcome) {
  ...
  output_file <- paste0("output_panel/pseudo_quant_fixed_", outcome, ".png")
  plot_coef <- ggplot(coefs_df, aes(x = year, y = coefficient, color = quantile, group = quantile)) +
    geom_line() +
    geom_hline(yintercept = 0, linetype = "twodash", color = "gray40") + 
    scale_x_continuous(breaks = unique(coefs_df$year)) + 
...
plot_counts <- plot_counts + labs(title = "Nombre de communes où le taux de propriétaires est >95%")
print(plot_counts)
...
quant_reg <- function(outcome) {
  ...
  output_file <- paste0("output_panel/quant_mob_", outcome, ".png")
...
quant_reg <- function(outcome) {
  ...
  output_file <- paste0("output_panel/quant_fixed_", outcome, ".png")
  plot_coef <- ggplot(coefs_df, aes(x = year, y = coefficient, color = quantile, group = quantile)) +
    geom_line() +
    geom_hline(yintercept = 0, linetype = "twodash", color = "gray40") + 
    scale_x_continuous(breaks = unique(coefs_df$year)) + 
