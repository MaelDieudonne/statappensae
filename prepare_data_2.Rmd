...
Enjeu : construire une base unique à partir de plusieurs bases, avec des bases annuelles pour les résultats électoraux, et des bases globales au format long pour les régresseurs, ce qui rend impossible de les fusionner directement.

Stratégie : itérer à partir des résultats électoraux. On commence par créer une table de destination avec l'ensemble des variables souhaitées. Puis pour chaque fichier électoral...
...
2.  On ajoute les contrôles de l'année correspondante (merge et renaming pour passer au format long)
...
```{r}
# Boucle sur les élections puis écriture du fichier
for (year in unique(candidats$Annee)) {
...
Construction d'une base limitée à une année, mais intégrant toutes les données disponibles en fin de période : criminalité, fiscalité, rsa, mais aussi typologie des communes.

On repart de la base précédente en isolant l'année 2017.
...
Les données cartographiques sont très volumineuses car très précises, mais aussi stables au cours du temps.

Plutôt que de les ajouter aux bases précédentes, ce qui les alourdirait considérablement, nous prévoyons de créer une fonction permettant de générer dynamiquement la base nécessaire.

Difficulté : shapefile des communes considère PLM dans leur intégralité. Faut utiliser un second shapefile pour récupérer les arrondissements. Qu'on utilise seulement pour Paris, car seule ville dont les arrondissements sont présents dans la base.
