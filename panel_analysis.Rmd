...
Tableau présentant la proportion de valeurs manquantes par outcome et par an.
...
for (outcome in relative_outcomes) {
  na_prop <- panel_data[, .(na_prop = count_na(get(outcome))), by = year]
  na_prop_table <- merge(na_prop_table, na_prop, by = "year", all.x = TRUE)
  setnames(na_prop_table, "na_prop", outcome)
}
...
rm(count_na, na_prop, na_prop_table, outcome)
```
...
On peut d'abord représenter l'évolution du taux national de propriétaires.
...
plot <- ggplot(propri_tot, aes(x = year, y = prop_ratio)) +
  ...
  labs(title = "Proportion de propriétaires en France, 1960-2022",
       x = "Année",
...
Ce graphique n'est pas le plus pertinent ici, car nous raisonnons à l'échelle des communes et non des individus. Représentons donc l'évolution du taux communal moyen de propriétaires.

```{r}
propri <- load_parquet("Proprietaires/proprietairescommunes.parquet")

propri_share <- data.table(year = numeric(), ppropri = numeric())
for (year in 1960:2022) {
  var_name <- paste0("ppropri", year)
  average <- mean(propri[[var_name]], na.rm = TRUE)
  propri_share <- rbind(propri_share, data.table(year = year, ppropri = average))
}

plot <- ggplot(propri_share, aes(x = year, y = ppropri)) +
  geom_line(color = "skyblue") +
  geom_point(color = "skyblue") +
  labs(title = "Taux communal moyen de propriétaires, 1965-2022",
       x = "Année",
       y = "%") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
...
rm(propri, propri_share, year, var_name, average)
```

Etude de la distribution / évolution des déciles

```{r}
quantiles_list <- list()
for (yr in unique(panel_data$year)) {
  panel_subset <- panel_data[panel_data$year == yr, ]
  quantiles <- quantile(panel_subset$ppropri, probs = seq(0, 1, length.out = 10), na.rm = TRUE)
...
quantiles_df <- quantiles_df[quantiles_df$quintile != "X0." & quantiles_df$quintile != "X100.", ]

ggplot(quantiles_df, aes(x = year, y = ppropri, color = as.factor(quintile), group = quintile)) +
  geom_line() +
  labs(x = "Year", y = "%", color = "Quintile") +
  ggtitle("Déciles du taux communal moyen de propriétaire")
```
