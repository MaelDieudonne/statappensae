...
```{r}
# A compléter pour les autres outcomes
coefficients <- relative_results$part_T1_model$coefficients
coefficients <- as.data.frame(coefficients)
coefficients <- coefficients[grep("propri$", rownames(coefficients)), , drop = FALSE]
years <- sub("^year(\\d+):.*", "\\1", rownames(coefficients))
coefficients$year <- years
rownames(coefficients) <- NULL

ggplot(data = coefficients, aes(x = year, y = coefficients)) +
  geom_line(aes(group = 1), color = "skyblue") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkblue") + 
  labs(x = "Année", y = "Coefficients", title = "Coefficients de ppropri sur part_T1 au cours du temps") +
  theme(plot.title = element_text(hjust = 0.5))

rm(coefficients, years)
```
...
```{r}
# A compléter pour les autres outcomes
coefficients <- relative_results$part_model$coefficients
coefficients <- as.data.frame(coefficients)
coefficients <- coefficients[grep("propri$", rownames(coefficients)), , drop = FALSE]
years <- sub("^year(\\d+):.*", "\\1", rownames(coefficients))
coefficients$year <- years
...
ggplot(data = coefficients, aes(x = year, y = coefficients)) +
  geom_line(aes(group = 1), color = "skyblue") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkblue") + 
  labs(x = "Année", y = "Coefficients", title = "Coefficients de ppropri sur part au cours du temps") +
  theme(plot.title = element_text(hjust = 0.5))
