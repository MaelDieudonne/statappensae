Le code ayant été écrit en python, il faut d'abord rendre son exécution possible en créant en environnement virtuel.
...
```{r}
# Le chunk précédent bugue parfois à l'importation de pyarrow. Il semble que reticulate n'arrive pas toujours à charger pyarrow. Il n'y a pas de solution miracle, on peut essayer :

reticulate::py_module_available("pyarrow") # Pour vérifier que pyarrow est bien installé
reticulate::py_install('pyarrow', pip = TRUE) # Pour le réinstaller avec pip plutôt que conda
install_pyarrow(envname = 'r-reticulate')
conda_remove('r-reticulate') # Pour réinitialiser l'environnement virtuel
miniconda_uninstall() # Pour partir d'une nouvelle installation de Python

# Autres pistes : 
# - utiliser une installation de python complète plutôt de miniconda
# - forcer une version précédente de pyarrow
```
