...
tryCatch({install_miniconda()},  error = function() {cat("Miniconda is already installed.")})
if (!('r-reticulate' %in% conda_list()$name)) conda_create('r-reticulate')
use_condaenv('r-reticulate')
```

```{r include=FALSE}
package_list <- c('pyarrow', 'chardet', 'pandas', 'requests', 's3fs', 'tqdm')
py_install(package_list)
rm(package_list)
```
