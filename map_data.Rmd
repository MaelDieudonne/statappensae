...
```{r}
# Téléchargement du fond de carte. Le serveur est très lent, d'où le timeout. Si nous faisons beaucoup de cartes, il faudra plutôt déposer les fichiers sur le bucket.
...
rm(root_name, file_names, extensions, files_to_extract, root_path, source_file, target_file)
```
...
output_file <- paste0("part_1981_dep", selected_dep, ".png")
...
rm(bbox, aspect_ratio, base_height, base_width, font_size, selected_dep, title)
```
...
rm(bbox, aspect_ratio, base_height, base_width, title_font_size)
```
