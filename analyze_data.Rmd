...
```{r}
prop_missing_communes_1981 <- sum(apply(pres1981[, !excluded_covariables, with = FALSE], 1, function(row) any(is.na(row)))) / nb_communes
print("% of missing communes because of nan")
print(prop_missing_communes_1981)

rows_with_nan <- pres1981[apply(pres1981[, !excluded_covariables, with = FALSE], 1, function(row) any(is.na(row))), ]
prop_missing_pop_1981 <- sum(rows_with_nan$pop1981, na.rm = TRUE) / pop_tot_1981
print("% of missing population because of nan")
print(prop_missing_pop_1981)
```